...
- [Собираем свой **object elf64 file**](#собираем-свой-object-elf64-file)
  ...
  - [5. Symtab entry:](#5-symtab-entry)
    ...
    - [Пример записи имени](#пример-записи-имени)
...
*Примечание: индекс(символьный) - индекс на первый символов *
...
![image](res/img/shstrtrab.png)
        ...
        Указан верный индекс, указывающий на название рассматриваемой секции - .symtab.
...
* sh_link: 0x0128 - 0x012B байты:
        ...
        sh_link = 0x04. Для того, чтобы осознать, что значит это число вернёмся к таблице для sh_link и sh_info. (ссылка ниже)
...
* r_info:
        ...
        Хранит в себе индекс таблицы имён (.symtab), по отношению к которому должно быть выполнено перемещение, 
