...
*(Автор очень сильно перегорел, пытаясь разобраться в этом дерьме с документацией времён полеозоя, которую при этом хер достанешь, поэтому автор настоятельно не рекомендует собирать свой объектный эльф файл, в особенности, если у вас нет много свободного времени, и за спиной не горят дедлайны. Хотя мой гайд, пусть он и неидеален, значительно облегчит раскопки информации для решение задачи, из-за которой читатель попал на этот репозиторий)*
